...
int
clua_seri_unpack(lua_State *L, int n, int nsz) {
	...
	if (lua_type(L, n) == LUA_TSTRING) {
		size_t sz;
		buffer = (void *)lua_tolstring(L, n, &sz);
		len = (int)sz;
	} else {
		buffer = lua_touserdata(L, n);
		len = luaL_checkinteger(L, nsz);
	}
	if (len == 0) {
		return 0;
	}
